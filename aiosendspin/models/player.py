...
class PlayerStatePayload(SendspinModel):
    ...
    Compensates for delay beyond the device's audio port (external speakers, amplifiers).
    Does not cover processing delays before the port (DAC latency, audio buffers), which
    the client compensates itself.
    """
