...
> **Note:** The CLI client is currently included in the `aiosendspin` library for development purposes. Once the Sendspin Protocol stabilizes, it will be moved to a separate repository and package. This will require users to uninstall `aiosendspin[cli]` and install the new CLI package separately.
...
```bash
uvx --from "aiosendspin[cli]" sendspin-cli
```
...
**With pip:**
```bash
pip install "aiosendspin[cli]"
```

**With uv:**
```bash
uv tool install "aiosendspin[cli]"
```
...
```bash
git clone https://github.com/Sendspin-Protocol/aiosendspin.git
cd aiosendspin
pip install ".[cli]"
```
...
The CLI client will automatically connect to a Sendspin server on your local network and be available for playback.
...
```bash
sendspin-cli --id my-client-1 --name "Kitchen"
sendspin-cli --id my-client-2 --name "Bedroom"
```

- `--id`: A unique identifier for this client (optional; defaults to `sendspin-cli-<hostname>`, useful for running multiple instances on one computer)
...
```bash
sendspin-cli --list-audio-devices
```
...
```bash
sendspin-cli --static-delay-ms -100
```
