...
class TestGroupStartStream:
    ...
    def test_start_stream_can_skip_playback_state_update(
        self,
        mock_server: MagicMock,
        mock_client: MagicMock,
    ) -> None:
        """start_stream(update_playback_state=False) should preserve current state."""
        group = SendspinGroup(mock_server, mock_client)

        group.start_stream(update_playback_state=False)
